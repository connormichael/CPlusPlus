...
int main()
{
   ...
   xConversion = xRange / NUM_ROWS;
   yConversion = yRange / NUM_COLS;
   ...
   for (k = 0; k < 3; k++)
   {
      row = starArray[k].getX() / xConversion;
      col = starArray[k].getY() / yConversion;

      if (starArray[k].getRank() > 5)
         starMap.set(row, col, '1');
      else
         starMap.set(row, col, '*');
   }
